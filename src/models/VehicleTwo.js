...
// Vehicle History sub-schema
const v5cCertificateSchema = new mongoose.Schema({
  CertificateDate: { type: Date, required: false }
}, { _id: false });

const keeperChangesSchema = new mongoose.Schema({
  DateOfTransaction: { type: Date, required: false },
  NumberOfPreviousKeepers: { type: Number, required: false },
  DateOfLastKeeperChange: { type: Date, required: false }
}, { _id: false });

const colourChangeDetailsSchema = new mongoose.Schema({
  CurrentColour: { type: String, required: false },
  NumberOfPreviousColours: { type: Number, required: false },
  OriginalColour: { type: String, required: false },
  LastColour: { type: String, required: false },
...
const vehicleHistorySchema = new mongoose.Schema({
  ...
  NumberOfPreviousKeepers: { type: Number, required: false },
  V5CCertificateList: [{ type: v5cCertificateSchema, required: false }],
  KeeperChangesCount: { type: Number, required: false },
  VicCount: { type: Number, required: false },
...
const smmtDetailsSchema = new mongoose.Schema({
  ...
  TerminateDate: { type: String, required: false },
...
const vehicleTwoSchema = new mongoose.Schema({
    ...
    VehicleHistory: { type: vehicleHistorySchema, required: false },
